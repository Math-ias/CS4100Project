...
def zipped_x_o_value_to_character(tup):
    ...
    """
    x_placed, o_placed, r_placed = tup
    assert not (x_placed and o_placed and r_placed)
    if x_placed:
        return 'X'
    if o_placed:
        return 'O'
...
def flattened_characters(game):
    ...
    return list(map(zipped_x_o_value_to_character,
                    zip(game[0].flatten(), game[1].flatten(), game[2].flatten())))
...
def game_loop():
    ...
    xs_turn = 0  # 0 when it's X's turn, 1 when O, 2 when R.
    ...
    with term.fullscreen(), term.cbreak():  # We capture the full screen and don't print out input.
        while not tictactoe.game_over_3d(current_game):  # We go until the game is over.
            ...
            current_game[xs_turn][action] = True
            xs_turn = (xs_turn + 1) % 3
